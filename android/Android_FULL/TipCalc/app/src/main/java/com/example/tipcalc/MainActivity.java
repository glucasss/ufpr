...
public class MainActivity extends AppCompatActivity {
    ...
    private TextView totalPessoasTextView;
    private int currentCustomPessoas;
    private TextView totalPessoasEditText;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SeekBar customPessoas = (SeekBar)findViewById(R.id.pessoasSeekBar);
        totalPessoasTextView = (TextView)findViewById(R.id.totalPessoasTextView);
        totalPessoasEditText = (EditText)findViewById(R.id.totalPessoasEditText);

        currentCustomPercent = customSeekBar.getProgress();
        currentCustomPessoas = customPessoas.getProgress();

        billEditText.addTextChangedListener(billTextWatcher);

        customSeekBar.setOnSeekBarChangeListener(customSeekBarListener);
        customPessoas.setOnSeekBarChangeListener(customPessoasListener);
    }
    ...
    private void updateCustomPessoas(){
        totalPessoasTextView.setText(currentCustomPessoas+"");

        double pessoasTotalAmount = (currentBillTotal * 1.1) / currentCustomPessoas;

        totalPessoasEditText.setText(String.format("%.02f", pessoasTotalAmount));
    }
    ...
    private SeekBar.OnSeekBarChangeListener customPessoasListener =
            new SeekBar.OnSeekBarChangeListener(){
                @Override
                public void onProgressChanged(SeekBar seekbar, int progress, boolean fromUser){
                    currentCustomPessoas = progress;
                    updateCustomPessoas();
                }
